window.set_form_data = function (selector, data) {
    ...
    for (const key in data) {
        if (Object.hasOwnProperty.call(data, key)) {
            ...
            let select = document.querySelector(`${selector} select[name="${key}"]`) || document.querySelector(`${selector} input[id="${key}"]`);
            let textarea = document.querySelector(`${selector} textarea[name="${key}"]`) || document.querySelector(`${selector} input[id="${key}"]`);
